...
## 安装Git
参考教程：[Git教程](https://www.leolan.top/posts/41310/)
...
---
**如果是在另一台电脑上配置环境（远程仓库中有项目文件）只需要安装Node.js、Git、Markdown编辑器，之后`npm install -g hexo-cli`安装hexo后配置Git**
...
cat ~/.ssh/id_rsa.pub
```
**把密钥添加到Coding和Github上，再`git clone 仓库地址（建议用ssh地址）`就直接可以正常使用了。**
...
**Travis CI 持续集成自动部署Hexo**：https://www.karlzhou.com/2016/05/28/travis-ci-deploy-blog/
